...
use crate::types::ExecutionHeaderInfo;
use eth_types::{
    eth2::{
...
pub mod pallet {
    ...
    pub trait Config: frame_system::Config + pallet_balances::Config {
        ...
        type SubmitterUpdateBound: Get<u32>;
    }
    ...
    pub(super) type MinSubmitterBalance<T: Config> = StorageValue<_, T::Balance, ValueQuery>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn init(
            ...
            let min_storage_balance_for_submitter =
                Self::calculate_min_storage_balance_for_submitter(max_submitted_blocks_by_account);
            if typed_chain_id == TypedChainId::Evm(1) {
                    ...
                    (verify_bls_signatures) || trusted_signer.is_some(),
                    // The client can't be executed in the trustless mode without BLS sigs verification on Mainnet
            ...
            if let Some(account) = trusted_signer.clone() {
                TrustedSigner::<T>::put(account);
            }

            Paused::<T>::insert(typed_chain_id, false);
            ValidateUpdates::<T>::insert(typed_chain_id, validate_updates);
            VerifyBlsSignatures::<T>::insert(typed_chain_id, verify_bls_signatures);
            HashesGcThreshold::<T>::insert(typed_chain_id, hashes_gc_threshold);
            MaxUnfinalizedBlocksPerSubmitter::<T>::insert(
                typed_chain_id,
                max_submitted_blocks_by_account,
            );
            MinSubmitterBalance::<T>::put(min_storage_balance_for_submitter);
            FinalizedBeaconHeader::<T>::insert(typed_chain_id, finalized_beacon_header);
            FinalizedExecutionHeader::<T>::insert(typed_chain_id, finalized_execution_header_info);
            CurrentSyncCommittee::<T>::insert(typed_chain_id, current_sync_committee);
...
impl<T: Config> Pallet<T> {
    ...
    pub fn min_storage_balance_for_submitter(&self) -> T::Balance {
        Self::min_submitter_balance()
    }
