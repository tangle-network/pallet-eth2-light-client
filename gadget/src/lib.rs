...
pub async fn ignite_lc_relayer(ctx: LightClientRelayerContext)-> anyhow::Result<()>{
	...
	let task = || async {
		println!("Starting light client relayer");
		let maybe_client = ctx.clone().substrate_provider().await;
		let api_client = match maybe_client {
			Ok(client) => client,
			Err(err) => {
				tracing::error!(
					"Failed to connect with substrate client, retrying...!"
				);
		...
		let mut eth_pallet =
		EthClientPallet::new(api_client, ctx.lc_relay_config.ethereum_network.as_typed_chain_id());
		let mut relay = Eth2SubstrateRelay::init(&ctx.lc_relay_config, Box::new(eth_pallet.clone())).await;
...
pub async fn start_gadget(relayer_params: Eth2LightClientParams) {
	...
	ignite_lc_relayer(ctx.clone()).await.expect("failed to start light client relayer");
	// watch for signals
    let mut ctrlc_signal = unix::signal(unix::SignalKind::interrupt()).expect("failed to register ctrlc handler");
    let mut termination_signal = unix::signal(unix::SignalKind::terminate()).expect("failed to register termination handler");
    let mut quit_signal = unix::signal(unix::SignalKind::quit()).expect("failed to register quit handler");
    let shutdown = || {
