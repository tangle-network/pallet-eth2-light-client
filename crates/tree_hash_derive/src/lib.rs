	...
	struct_data
		...
		.filter_map(|f| {
			if should_skip_hashing(f) {
				...
				let ident =
					f.ident.as_ref().expect("tree_hash_derive only supports named struct fields");
...
fn compute_union_selectors(num_variants: usize) -> Vec<u8> {
	...
	let highest_selector =
		union_selectors.last().copied().expect("0-variant union is not permitted");
