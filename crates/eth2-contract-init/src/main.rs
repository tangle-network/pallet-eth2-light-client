...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let signer = setup_signer(&config).await.unwrap();
	let mut eth_client_contract = EthClientPallet::new(api, signer);
	init_pallet(&config, &mut eth_client_contract).expect("Error on contract initialization");
