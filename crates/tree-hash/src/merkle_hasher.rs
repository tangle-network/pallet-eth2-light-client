...
pub struct MerkleHasher {
	...
	/// heap. Each half-node is 224 bytes, so this smallvec may store 1792 bytes on the stack.
...
mod test {
	...
	fn compare_with_reference(leaves: &[Hash256], depth: usize) {
		let reference_bytes =
			leaves.iter().flat_map(|hash| hash.as_bytes()).copied().collect::<Vec<_>>();
	...
	fn compare_reference_with_len(leaves: u64, depth: usize) {
		let leaves = (0..leaves).map(Hash256::from_low_u64_be).collect::<Vec<_>>();
	...
	fn compare_new_with_leaf_count(num_leaves: u64, depth: usize) {
		let leaves = (0..num_leaves).map(Hash256::from_low_u64_be).collect::<Vec<_>>();
		...
		let from_num_leaves = {
			...
			for leaf in leaves.iter() {
				m.process_leaf(leaf.as_bytes()).expect("should process leaf");
			}
			m.finish()
		};
