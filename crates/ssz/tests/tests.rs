...
mod round_trip {
	...
	#[test]
	fn vec_of_h256() {
		let items: Vec<Vec<H256>> =
			vec![vec![], vec![H256::zero(), H256::from([1; 32]), H256::random()]];
...
mod derive_macro {
	...
	fn two_variable_trans() {
		let trans_a = TwoVariableTrans::A(VariableA { a: 1, b: vec![2, 3] });
		let trans_b = TwoVariableTrans::B(VariableB { a: vec![1, 2], b: 3 });

		assert_encode(&trans_a, &[1, 5, 0, 0, 0, 2, 3]);
		assert_encode(&trans_b, &[5, 0, 0, 0, 3, 1, 2]);

		assert_encode(&TwoVariableTransStruct { a: trans_a }, &[4, 0, 0, 0, 1, 5, 0, 0, 0, 2, 3]);
		assert_encode(&TwoVariableTransStruct { a: trans_b }, &[4, 0, 0, 0, 5, 0, 0, 0, 3, 1, 2]);
	}

	#[test]
	fn two_variable_union() {
		let union_a = TwoVariableUnion::A(VariableA { a: 1, b: vec![2, 3] });
		let union_b = TwoVariableUnion::B(VariableB { a: vec![1, 2], b: 3 });
