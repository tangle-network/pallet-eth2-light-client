...
use consensus_types::{
    compute_domain, compute_signing_root, get_participant_pubkeys,
    DOMAIN_SYNC_COMMITTEE, MIN_SYNC_COMMITTEE_PARTICIPANTS,
};
use eth_types::eth2::{BeaconBlockHeader, LightClientUpdate, SyncCommittee, Epoch, ForkVersion, Slot};
use eth_types::H256;
use consensus_types::compute_fork_version_by_slot;
...
mod tests {
    ...
    fn smoke_verify_finality_update() {
        ...
        let network: NetworkConfig = NetworkConfig::new(
            &Network::from_str(&config.network_name).unwrap(),
        );
