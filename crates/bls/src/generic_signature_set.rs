	...
	AggSig: TAggregateSignature<Pub, AggPub, Sig> + Clone,
{
	fn from(sig: &'a GenericSignature<Pub, Sig>) -> Self {
		...
		WrappedSignature { aggregate: Cow::Owned(aggregate) }
	}
}
	...
	AggSig: Clone,
{
	fn from(aggregate: &'a GenericAggregateSignature<Pub, AggPub, Sig, AggSig>) -> Self {
		WrappedSignature { aggregate: Cow::Borrowed(aggregate) }
	}
}

/// A generic way to represent a signature across a message by multiple public keys.
///
/// This struct is primarily useful in a collection (e.g., `Vec<GenericSignatureSet>`) so we can
/// perform multiple-signature verification which is much faster than verifying each signature
/// individually.
#[derive(Clone)]
	...
	AggSig: TAggregateSignature<Pub, AggPub, Sig> + Clone,
{
	...
	pub fn multiple_pubkeys(
		...
		Self { signature: signature.into().aggregate, signing_keys, message, _phantom: PhantomData }
	}

	/// Returns `true` if `self.signature` is a signature across `self.message` by
	/// `self.signing_keys`.
	pub fn verify(self) -> bool {
		let pubkeys = self.signing_keys.iter().map(|pk| pk.as_ref()).collect::<Vec<_>>();
