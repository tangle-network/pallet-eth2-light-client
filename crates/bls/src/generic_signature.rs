	...
	Sig: TSignature<Pub>,
{
	...
	/// meaningful.
	pub fn empty() -> Self {
		Self { point: None, is_infinity: false, _phantom: PhantomData }
	}
	...
	pub(crate) fn from_point(point: Sig, is_infinity: bool) -> Self {
		Self { point: Some(point), is_infinity, _phantom: PhantomData }
	}
	...
	pub fn deserialize(bytes: &[u8]) -> Result<Self, Error> {
		let point =
			if bytes == &NONE_SIGNATURE[..] { None } else { Some(Sig::deserialize(bytes)?) };

		Ok(Self { point, is_infinity: bytes == &INFINITY_SIGNATURE[..], _phantom: PhantomData })
	}
}
