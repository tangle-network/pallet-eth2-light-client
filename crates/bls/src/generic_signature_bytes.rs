...
impl<Pub, Sig> GenericSignatureBytes<Pub, Sig> {
	...
	pub fn deserialize(bytes: &[u8]) -> Result<Self, Error> {
		if bytes.len() == SIGNATURE_BYTES_LEN {
			...
			Err(Error::InvalidByteLength { got: bytes.len(), expected: SIGNATURE_BYTES_LEN })
		}
	}
}
...
/// Serializes the `GenericSignature` in compressed form, storing the bytes in the newly created
/// `Self`.
