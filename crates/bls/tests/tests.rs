...
macro_rules! test_suite {
	($impls: ident) => {
		...
		#[test]
		fn infinity_agg_sig() {
			assert_eq!(&AggregateSignature::infinity().serialize()[..], &INFINITY_SIGNATURE[..]);
			assert_eq!(
		...
		fn partial_eq_infinity_agg_sig() {
			assert_eq!(AggregateSignature::infinity(), AggregateSignature::infinity())
		}
		...
		fn infinity_signature_is_invalid_with_standard_pubkey() {
			SignatureTester::default().infinity_sig().assert_verify(false)
		}
		...
		impl AggregateSignatureTester {
			...
			pub fn aggregate_infinity_sig(mut self) -> Self {
				self.sig.add_assign(&Signature::deserialize(&INFINITY_SIGNATURE[..]).unwrap());
				self
			}
		...
		impl SignatureSetTester {
			...
			pub fn run_checks(self) {
				...
				let should_be_valid =
					self.owned_sets.iter().all(|owned_set| owned_set.should_be_valid);
				...
				assert_eq!(
					verify_signature_sets(signature_sets.iter(), [1u8; 32]),
					should_be_valid
				);
			}
		}
		...
		fn signature_set_1_invalid_set() {
			SignatureSetTester::default().push_invalid_set().run_checks()
		}
		...
		fn signature_set_1_valid_set_with_128_signers() {
			SignatureSetTester::default().push_valid_set(128).run_checks()
		}

		#[test]
		fn signature_set_2_valid_set_with_one_signer_each() {
			SignatureSetTester::default().push_valid_set(1).push_valid_set(1).run_checks()
		}

		#[test]
		fn signature_set_2_valid_set_with_2_signers_each() {
			SignatureSetTester::default().push_valid_set(2).push_valid_set(2).run_checks()
		}

		#[test]
		fn signature_set_2_valid_set_with_1_invalid_set() {
			SignatureSetTester::default().push_valid_set(2).push_invalid_set().run_checks()
		}
