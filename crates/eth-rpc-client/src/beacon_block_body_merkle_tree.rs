...
impl BeaconBlockBodyMerkleTree {
	...
	pub fn new(beacon_block_body: &BeaconBlockBody<MainnetEthSpec>) -> Self {
		...
		Self(MerkleTree::create(&leaves, Self::BEACON_BLOCK_BODY_TREE_DEPTH))
	}
}
...
/// The hash of the root of `ExecutionPlayloadMerkleTree` is the 9th leaf in BeaconBlockBody Merkle
/// Tree.
...
mod tests {
	use crate::{
		...
		utils::read_json_file_from_data_dir,
	};
	use tree_hash::TreeHash;
	...
	fn test_execution_payload_merkle_tree() {
		...
		assert_eq!(execution_payload.tree_hash_root(), execution_payload_merkle_tree.0.hash());
